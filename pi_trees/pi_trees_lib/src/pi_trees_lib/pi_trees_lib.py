...
class Task(object):
    ...
    def announce(self):
        ...
        rospy.loginfo("Executing task " + str(self.name))
...
class Selector(Task):
    ...
    def run(self):
        for c in self.children:
            if  c.status == TaskStatus.SUCCESS or c.status == TaskStatus.FAILURE:
                continue
...
class ParallelSelector(Task):
    """
    """
    def __init__(self, name):
        super(ParallelSelector, self).__init__(name)
        self.node_type = "ParallelSelector"
    ...
    def run(self):
        ...
        if fail_count >= len(self.children):
            self.reset()
            return  TaskStatus.FAILURE
...
class LoopInf(Task):
    ...
    def __init__(self, name):
        super(LoopInf, self).__init__(name)
        self.node_type = "LoopInf"
        self.run_ind    = 0

    def run(self):
        while True:
            self.children[self.run_ind].status = self.children[self.run_ind].run()

            if self.children[self.run_ind].status == TaskStatus.RUNNING:
                return  TaskStatus.RUNNING

            self.run_ind    += 1

            if self.run_ind >= len(self.children):
                self.reset()
                self.run_ind    = 0

    def reset(self):
        super(LoopInf, self).reset()
        self.run_ind    = 0
...
class AutoRemoveSequence(Task):
    ...
    def __init__(self, name, *args, **kwargs):
        ...
        self.node_type = "AutoRemoveSequence"
