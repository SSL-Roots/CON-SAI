...
class FormatConverter:
    ...
    def tableToRosmsg(self, ros_current_time):
        ...
        for obs in self.table:
            if obs['color'] == 'ball':
                vision_packet   = self._observationToVisionPacket(obs)
    ...
    def _observationToVisionPacket(self, observation):
        ...
        vision_packet.pose  = self._observationToPose(observation)

        return  vision_packet


    def _observationToPose(self, observation):
        pose = Pose()
        ...
        pose.position.x = observation['x'] * self._TO_METER
        pose.position.y = observation['y'] * self._TO_METER
        pose.position.z = observation['z'] * self._TO_METER
