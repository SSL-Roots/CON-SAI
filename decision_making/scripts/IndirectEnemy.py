...
from CheckRefboxCommand import IsINDIRECT_FREE_ENEMY
import GoalKeeper
import Attacker
import Analyst
import InPlay
...
class IndirectEnemy(ParallelAll):
    def __init__(self, name):
        super(IndirectEnemy , self).__init__(name)

        IS_INDIRECT_ENEMY = IsINDIRECT_FREE_ENEMY('Is Indirect Enemy')
        INPLAY_SWITCH = InplaySwitch("InplaySwitch")

        self.add_child(IS_INDIRECT_ENEMY)
